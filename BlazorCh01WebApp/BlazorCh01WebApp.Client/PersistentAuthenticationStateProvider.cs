...
namespace BlazorCh01WebApp.Client
{
    internal class PersistentAuthenticationStateProvider : AuthenticationStateProvider
    {
        ...
        {
            if (!state.TryTakeFromJson<UserInfo>(nameof(UserInfo), out
            var userInfo) || userInfo is null)
            {
                return;
            }
            List<Claim> claims = new();
            claims.Add(new Claim(ClaimTypes.NameIdentifier, userInfo.
            UserId));
